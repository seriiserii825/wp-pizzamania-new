		...
		<span class="btn3"><?php pll_e('Детали'); ?></span>
				...
				<div class="initPopupForm" style="display:none">
					...
					<div class="block-stl6" style="margin-top:30px;">
							...
							<p class="price"><span><?php the_field('price') ?> <?php pll_e('Лей'); ?></span><?php if(get_field('old_price')) : ?> <small><del><?php the_field('old_price') ?> <?php pll_e('Лей'); ?></del></small><?php endif; ?></p>
...
?>
								...
								<?php } ?>
									...
									<div style="clear:both"></div>
							<p class="ab-txt"><?php pll_e('Вес'); ?>: <?php the_field('weight') ?> <?php pll_e('г'); ?>.<br><?php pll_e('Состав'); ?>: <?php the_field('sostav') ?></p>
							<p class="ab-txt green"><?php pll_e('Товар добавлен в корзину') ?>.</p>

							<div class="form-group btn-block">
								<a href="#" class="btn1 stl2 add_item"  data-id="<?php the_ID(); ?>" data-title="<?php echo get_the_title() ?>" data-price="<?php the_field('price') ?>" data-img="<?php the_field('photo') ?>" data-type=""><?php pll_e('В корзину') ?></a>
